 ...
 *
 * Delegates the game-loop logic to the useSoloGame hook.
 * All trade actions come from GameContext.
 */
...
const SetupScreen = () => {
  const { selectMode, startSoloGame } = useGame();
  ...
  return (
    <div className="flex min-h-[calc(100vh-80px)] items-center justify-center p-6">
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900/50 p-8 backdrop-blur-xl">
        ...
        <p className="mb-8 text-slate-400">Start with {fmt(INITIAL_CAPITAL)}. Can you beat the market?</p>
...
const ReflectionScreen = () => {
  const { gameData, gameStatus, setGameStatus } = useGame();
...
const FinishedScreen = () => {
  const { netWorth, resetGame } = useGame();
...
const Game = () => {
  ...
  // Drives currentIndex / timeLeft while playing (no-op otherwise)
  useSoloGame();

  if (gameStatus === 'setup') return <SetupScreen />;
  if (gameStatus === 'reflection') return <ReflectionScreen />;
  if (gameStatus === 'finished') return <FinishedScreen />;
